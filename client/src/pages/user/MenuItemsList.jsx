    ...
    <List strongIos dividersIos>
      {category?.map((category) => (
        <div key={category}>
          <div className="flex w-full border-2 p-2 ">
            <div className="ml-5" id={category}>{category}</div>
      ...
      <div class="fixed z-50">
        <div class="fixed bottom-20 right-0">
          <button
            onClick={() => showlist()}
            class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-6 px-6 mr-5 rounded-full"
          >
            <span>
      ...
      {showListStatus ? (
        <div class="fixed  z-50">
          <div class="fixed bottom-8 right-0">
            <ul class="bg-orange-100 border rounded shadow-md absolute right-0 bottom-12 mr-4 w-64 h-72 overflow-y-scroll">
            {category?.map((category) => (
              <li
                onClick={() => {
